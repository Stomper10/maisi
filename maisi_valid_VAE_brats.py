...
def main():
    ...
    # valid_t1n_files = glob.glob(os.path.join(args.valid_label_dir, "*", "*-t1n.nii.gz"))
    valid_t1n_files = glob.glob(os.path.join(args.valid_label_dir, "*", "*.nii.gz"))
    # valid_t1n_files = glob.glob(os.path.join(args.train_label_dir, "*", "*-t1n.nii.gz"))[500:1000] # recon
    # valid_files = [{"image": f} for f in sorted(valid_t1n_files)]
    valid_files = [{"image": f} for f in sorted(valid_t1n_files) if "seg" not in f]
    ...
    val_transform = Compose([
        ...
        Resized(
            keys=["image"],
            spatial_size=(256,256,128),     # 원하는 해상도 (tuple 또는 list)
            size_mode="all",
        ...
        with torch.no_grad():
            with autocast("cuda", enabled=args.amp):
                ...
                output_path = "{0}/recon_size{1:d}x{2:d}x{3:d}_{4}.nii.gz".format(
                    "/leelabsg/data/ex_MAISI/E0_BraTS_temp/recon/whole",
                    data.shape[0],
                    data.shape[1],
                    data.shape[2],
                    timestamp,
    ...
    Image.fromarray(vis_image).save("/shared/s1/lab06/wonyoung/maisi/images/vis_image_brats.png")
    Image.fromarray(vis_recon_image).save("/shared/s1/lab06/wonyoung/maisi/images/vis_recon_image_brats.png")
