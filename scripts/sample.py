...
def ldm_conditional_sample_one_image(
    ...
    with torch.no_grad(), torch.amp.autocast("cuda"):
        ...
        ###
        v_min = torch.quantile(synthetic_images.float(), 0.0)
        v_max = torch.quantile(synthetic_images.float(), 0.995)
        b_min, b_max = 0.0, 1.0
        synthetic_images = (synthetic_images - v_min) / (v_max - v_min)
        synthetic_images = synthetic_images * (b_max - b_min) + b_min
