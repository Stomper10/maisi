...
def calculate_fid_real(args):
    ...
    args.train_label_dir = "/leelabsg/data/BraTS25/ASNR-MICCAI-BraTS2023-GLI-Challenge-TrainingData/train"
    args.batch_size = 2

    model = get_feature_extractor()
    
    train_t1n_files = glob.glob(os.path.join(args.train_label_dir, "*", "*-t1n.nii.gz"))[500:1000]
    train_files = [{"image": f} for f in sorted(train_t1n_files)]
    datasets = {
        1: {
            "data_name": "Brats2023-GLI",
    ...
    #np.save("/shared/s1/lab06/wonyoung/maisi/eval/brats_first500.npy", act)
    ...
    # np.save("/shared/s1/lab06/wonyoung/maisi/eval/brats_first500_m1.npy", m)
    # np.save("/shared/s1/lab06/wonyoung/maisi/eval/brats_first500_s1.npy", s)
    # np.load("/shared/s1/lab06/wonyoung/maisi/eval/brats_first500_m1ssssss.npy")

    m1 = np.load("/shared/s1/lab06/wonyoung/maisi/eval/brats_first500_m1.npy")
    s1 = np.load("/shared/s1/lab06/wonyoung/maisi/eval/brats_first500_s1.npy")
...
def calculate_fid_fake(args):
    #assert os.path.exists("./results/fid/m_real_"+args.real_suffix+str(args.fold)+".npy")
    args.data_dir = "/leelabsg/data/ex_MAISI/E0_BraTS_temp/outputs" # gen
    args.data_dir = "/leelabsg/data/ex_MAISI/E0_BraTS_temp/recon/whole" # recon
    args.batch_size = 2

    model = get_feature_extractor()

    train_files = [{"image": os.path.join(args.data_dir, image_name)} for image_name in os.listdir(args.data_dir) if "image.nii.gz" in image_name][:500]
    train_files = [{"image": os.path.join(args.data_dir, image_name)} for image_name in os.listdir(args.data_dir) if "nii.gz" in image_name][:500]
    datasets = {
        1: {
    ...
    train_transform = Compose([
        ...
        Resized(
            keys=["image"],
            spatial_size=(240,240,155),     # 원하는 해상도 (tuple 또는 list)
            size_mode="all",
...
if __name__ == '__main__':
    ...
    #act_fake = np.load("/shared/s1/lab06/wonyoung/maisi/eval/brats_first500.npy")
