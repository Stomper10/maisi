...
def load_filenames(data_list_path: str, type: str) -> list:
    ...
    return [_item["image"].split("/")[4][:7] + "_emb.nii.gz" for _item in filenames] ###
...
def train_one_epoch(
    ...
    amp: bool = True,
) -> tuple[torch.Tensor, int]: # [수정] 반환 값에 step 추가 명시
    """
    ...
    for train_data in train_loader:
        ...
        # print('### train_data["cond"]:', train_data["cond"], flush=True)
        # print('### train_data["cond"].shape:', train_data["cond"].unsqueeze(1).shape, flush=True) ###
        # context = train_data["cond"].unsqueeze(1).to(device) ###
        ...
        if SHUTDOWN_REQUESTED:
            if local_rank == 0:
                ...
                save_checkpoint(
                    ...
                    step=step,
                    unet=unet,
                    loss_torch_epoch=loss.item(), # 현재 스텝의 loss
                    val_loss_epoch=float('inf'), # validation 전이므로 임시 값
...
def diff_model_train(
    ...
    if local_rank == 0:
        ...
        wandb.init(project="maisi_unet", config=args)
    ...
    for _i in range(len(filenames_valid)):
        ...
        valid_files_i = {"image": str_img, "spacing": str_info, "cond": str_info} ###
        if include_body_region:
