...
def main():
    ...
    autoencoder.post_quant_conv = autoencoder.post_quant_conv.to(device)
    ...
    if rank == 0: 
        ...
        logger.info("VAE Encoder/quant layers discarded from VRAM.")
        logger.info("VAE Decoder and post_quant_conv moved to GPU frozen.")
    ...
    aux_loss_fn = FrequencyWeightedLoss(device=device)
    ...
    for step in progress_bar:
        ...
        with autocast(device_type="cuda", dtype=torch.float16, enabled=args.amp):
            ...
            spatial_loss, low_wavelet_loss, high_wavelet_loss = torch.tensor(0.0, device=device), torch.tensor(0.0, device=device), torch.tensor(0.0, device=device)
            ...
            if t_sample.item() < args.hwl_timestep_threshold:
                with torch.no_grad():
                    ...
                    target_x0_voxel_sample = batch["target_voxel_image"].to(device, non_blocking=True).unsqueeze(0)
                    ...
                    predicted_x0_latent = x_t_sample + t_reshaped_norm * v_pred_sample
                
                    # VAE Decoder로 Voxel Space 복원
                    predicted_x0_voxel = autoencoder.decode_stage_2_outputs(predicted_x0_latent / scale_factor)

                    # Voxel Space에서 주파수 손실 계산
                    spatial_loss, low_wavelet_loss, high_wavelet_loss = aux_loss_fn(predicted_x0_voxel, target_x0_voxel_sample)
                
                # 3. 최종 손실 결합                
                total_loss += (args.voxel_spatial_weight * spatial_loss + \
        ...
        spatial_loss = reduce_mean_scalar(spatial_loss) * args.gradient_accumulation_steps
        low_wavelet_loss = reduce_mean_scalar(low_wavelet_loss) * args.gradient_accumulation_steps
        high_wavelet_loss = reduce_mean_scalar(high_wavelet_loss) * args.gradient_accumulation_steps
                ...
                for val_batch in valid_loader:
                    ...
                    with autocast(device_type="cuda", dtype=torch.float16, enabled=args.amp):
                        ...
                        if t_sample.item() < args.hwl_timestep_threshold:
                            v_pred_sample = model_output[idx:idx+1]
                            x_t_sample = noisy_latent[idx:idx+1]
                            target_x0_voxel_sample = val_batch["target_voxel_image"].to(device, non_blocking=True).unsqueeze(0)
                            ...
                            val_epoch_loss["voxel_spatial_loss"] += spatial_loss
                            val_epoch_loss["voxel_low_wavelet_loss"] += low_wavelet_loss
                            val_epoch_loss["voxel_high_wavelet_loss"] += high_wavelet_loss
