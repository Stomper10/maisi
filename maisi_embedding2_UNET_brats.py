...
def process_file(
    ...
    """
    out_filename_base = filepath.replace(".nii.gz", "_emb.nii.gz")
    #out_filename_base = os.path.join(args.embedding_base_dir, out_filename_base)
    #out_filename = out_filename_base + "_emb.nii.gz"
    out_filename = os.path.join(args.embedding_base_dir, out_filename_base.split("/")[-1]) ###
